...
var direccionRET = '0xC96dfF22FEA4A52aec6385F73982C0bA4e1E3336';
...
$(document).ready(function () {
    ...
    $('#btnComprarPieza').click(comprarPieza);
    $('#btnRecibir').click(recibirPieza);
    $('#btnEnVenta').click(venderPieza);
    ...
    $('#tabla_piezas tbody').on('click', 'tr td', function () {
        ...
        if ( row.child.isShown() ) {
            ...
            $('#txtUPC').val(partSelected);
            $('#txtPRECIO').val(partSelectedPrice);
            if (partSelectedState == 7 ){
                btnComprar.disabled = false;
            }
            if (partSelectedState == 9 ){
                btnRecibir.disabled = false;
            }
            if (partSelectedState == 10 ){
    ...
    NumKeyboard();
    initWeb3();
    obtenerHistoricoEventosTodos('ForSaleByDistributor',t_prod);
...
function comprarPieza() {
    ...
    CPCContract.methods.purchaseItemByRetailer(serial).send({ from: miAddress, gas: 1000000 }, function (error, result) {
    console.log("Smart Contract Transaction sent");
    console.log(result);   
   });
   }
...
function recibirPieza() {
    ...
    miAddress = web3.eth.accounts.currentProvider.selectedAddress;
    console.log(miAddress);
    CPCContract.methods.receivedItemByRetailer(serial).send({ from: miAddress, gas: 1000000 }, function (error, result) {
    console.log("Smart Contract Transaction sent");
    console.log(result);   
...
function venderPieza() {
    ...
    miAddress = web3.eth.accounts.currentProvider.selectedAddress;
    console.log(miAddress);
    CPCContract.methods.sellItemByRetailer(serial, price).send({ from: miAddress, gas: 1000000 }, function (error, result) {
    console.log("Smart Contract Transaction sent");
    console.log(result);   
...
async function subscribirEventoForSaleByDistributor(table) {
    CPCContract.events.ForSaleByDistributor({
    }, function(error, event){ console.log(event); })
    .on("connected", function(subscriptionId){
        console.log(subscriptionId);
    })
