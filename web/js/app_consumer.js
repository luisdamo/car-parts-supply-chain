...
var direccionCON = '0x98f64CD4D3Dd97E4A2042C28348258952dBb3B92';
...
$(document).ready(function () {
    ...
    NumKeyboard();
    initWeb3();
    obtenerHistoricoEventosTodos('ForSaleByRetailer',t_prod);
    subscribirEventoForSaleByRetailer(t_prod);
});
...
function comprarPieza() {
    ...
    CPCContract.methods.purchaseItemByConsumer(serial).send({ from: miAddress, gas: 1000000 }, function (error, result) {
    console.log("Smart Contract Transaction sent");
    console.log(result);   
   });
   }
...
async function subscribirEventoForSaleByRetailer(table) {
    CPCContract.events.ForSaleByRetailer({
    }, function(error, event){ console.log(event); })
    .on("connected", function(subscriptionId){
        console.log(subscriptionId);
    })
