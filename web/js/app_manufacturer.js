...
var direccionFAB = '0x69b34e02259beB17F6ce570Ab5c218560D67Fca5';
...
var btnEnVenta = document.getElementById("btnEnVenta");
$(document).ready(function () {
    ...
    $('#txtFabricante').val(direccionFAB);
    $('#txtUPC').focus(function () {
        $write = $('#txtUPC');
        onlyNumeric = true;
    });
    ...
    $('#btnCrearPieza').click(crearPieza);
    btnEnVenta.disabled = true;
    $('#btnEnVenta').click(venderPiezaFabricante);
    $('#btnEnviar').click(enviarPiezaFabricante);
    $('#tabla_piezas tbody').on('click', 'tr td', function () {
        ...
        if ( row.child.isShown() ) {
            ...
            $('#txtUPC').val(partSelected);
            $('#txtPRECIO').val(partSelectedPrice);
            if (partSelectedState == 0 ){
                btnEnVenta.disabled = false;
            }
            if (partSelectedState == 2 ){
    ...
    NumKeyboard();
    initWeb3();
    obtenerHistoricoEventos(direccionFAB,'ProduceByManufacturer',t_prod);
    subscribirEventoProduceByManufacturer(direccionFAB, t_prod);
});
...
function initcontract() {
    ...
    web3.eth.getBlockNumber().then(testlog);
...
function crearPieza() {
    ...
    if ((serial == null) || (serial.length < 1)  || (serial < 1))  {
        serial = "";
        showtoast("Crear nueva pieza ", "Debe introducir código UPC de la pieza");
    }
    else {
        if ((part_type == null) || (part_type.length < 1)) {
            part_type = "";
            showtoast("Crear nueva pieza ", "Debe introducir el tipo pieza");
        }
        else {
            if ((price == null) || (price.length < 1)) {
                price = 0;
                showtoast("Crear nueva pieza ", "Debe introducir el precio de la pieza");
            }
            else {
            ...
            CPCContract.methods.produceItemByManufacturer(serial, part_type,notes, price).send({ from: miAddress, gas: 1000000 }, function (error, result) {
...
function venderPiezaFabricante() {
    var serial = $('#txtUPC').val();
    var part_type = $('#selTipo').val();
    var price = $('#txtPRECIO').val();
    var notes = $('#txtNotas').val();
    ...
    //Create part and send information to blockchain
    if(partSelectedState != 0) {
        showtoast("Poner pieza en venta ", "Verifique el estado actual de la pieza, no es posible poner en venta");
    } else{
        miAddress = web3.eth.accounts.currentProvider.selectedAddress;
        console.log(miAddress);
        CPCContract.methods.sellItemByManufacturer(serial, price).send({ from: miAddress, gas: 1000000 }, function (error, result) {
        console.log("Smart Contract Transaction sent");
        console.log(result);   
       });
    }
...
function enviarPiezaFabricante() {
    var serial = $('#txtUPC').val();
    
    //Create part and send information to blockchain
    if(partSelectedState != 2) {
        showtoast("Enviar pieza ", "Verifique el estado actual de la pieza, no es posible enviar");
    } else{
        miAddress = web3.eth.accounts.currentProvider.selectedAddress;
        console.log(miAddress);
        CPCContract.methods.shippedItemByManufacturer(serial).send({ from: miAddress, gas: 1000000 }, function (error, result) {
        console.log("Smart Contract Transaction sent");
        console.log(result);   
       });
    }
...
function testlog(val){
    blocknum = val;
    console.log('blocknum actual:' + blocknum);
}
...
async function obtenerHistoricoEventosTodos(event, table) {
    ...
    .then(function(events){
            ...
            console.log('default block:' + CPCContract.defaultBlock);
            console.log('bloque ini:' + blockini);
            console.log('bloque fin:' + blockfin);
